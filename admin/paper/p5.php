...
$rmsql = "SELECT * FROM `np` WHERE `id`=5";
...
if (isset($_FILES['p5'])) {
    ...
    $img_name = $_FILES['p5']['name'];
    $img_name = preg_replace("/\s+/", "_", $img_name);
    $img_size = $_FILES['p5']['size'];
    $img_temp_name = $_FILES['p5']['tmp_name'];
    $img_type = $_FILES['p5']['type'];
...
// $sql = "INSERT INTO `np`(`id`, `img_name`) VALUES (5,'{$img_name}')";
$sql = "UPDATE `np` SET `img_name`='{$img_name}' WHERE `id`=5";
