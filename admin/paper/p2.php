...
$rmsql = "SELECT * FROM `np` WHERE `id`=2";
...
if (isset($_FILES['p2'])) {
    ...
    $img_name = $_FILES['p2']['name'];
    $img_name = preg_replace("/\s+/", "_", $img_name);
    $img_size = $_FILES['p2']['size'];
    $img_temp_name = $_FILES['p2']['tmp_name'];
    $img_type = $_FILES['p2']['type'];
...
// $sql = "INSERT INTO `np`(`id`, `img_name`) VALUES (2,'{$img_name}')";
$sql = "UPDATE `np` SET `img_name`='{$img_name}' WHERE `id`=2";
