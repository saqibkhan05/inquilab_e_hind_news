...
$rmsql = "SELECT * FROM `np` WHERE `id`=6";
...
if (isset($_FILES['p6'])) {
    ...
    $img_name = $_FILES['p6']['name'];
    $img_name = preg_replace("/\s+/", "_", $img_name);
    $img_size = $_FILES['p6']['size'];
    $img_temp_name = $_FILES['p6']['tmp_name'];
    $img_type = $_FILES['p6']['type'];
...
// $sql = "INSERT INTO `np`(`id`, `img_name`) VALUES (6,'{$img_name}')";
$sql = "UPDATE `np` SET `img_name`='{$img_name}' WHERE `id`=6";
