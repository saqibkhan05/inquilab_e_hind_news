...
$rmsql = "SELECT * FROM `np` WHERE `id`=4";
...
if (isset($_FILES['p4'])) {
    ...
    $img_name = $_FILES['p4']['name'];
    $img_name = preg_replace("/\s+/", "_", $img_name);
    $img_size = $_FILES['p4']['size'];
    $img_temp_name = $_FILES['p4']['tmp_name'];
    $img_type = $_FILES['p4']['type'];
...
// $sql = "INSERT INTO `np`(`id`, `img_name`) VALUES (4,'{$img_name}')";
$sql = "UPDATE `np` SET `img_name`='{$img_name}' WHERE `id`=4";
