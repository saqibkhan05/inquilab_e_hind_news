...
$rmsql = "SELECT * FROM `np` WHERE `id`=8";
...
if (isset($_FILES['p8'])) {
    ...
    $img_name = $_FILES['p8']['name'];
    $img_name = preg_replace("/\s+/", "_", $img_name);
    $img_size = $_FILES['p8']['size'];
    $img_temp_name = $_FILES['p8']['tmp_name'];
    $img_type = $_FILES['p8']['type'];
...
// $sql = "INSERT INTO `np`(`id`, `img_name`) VALUES (8,'{$img_name}')";
$sql = "UPDATE `np` SET `img_name`='{$img_name}' WHERE `id`=8";
