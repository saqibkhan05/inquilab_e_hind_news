...
$rmsql = "SELECT * FROM `np` WHERE `id`=7";
...
if (isset($_FILES['p7'])) {
    ...
    $img_name = $_FILES['p7']['name'];
    $img_name = preg_replace("/\s+/", "_", $img_name);
    $img_size = $_FILES['p7']['size'];
    $img_temp_name = $_FILES['p7']['tmp_name'];
    $img_type = $_FILES['p7']['type'];
...
// $sql = "INSERT INTO `np`(`id`, `img_name`) VALUES (7,'{$img_name}')";
$sql = "UPDATE `np` SET `img_name`='{$img_name}' WHERE `id`=7";
